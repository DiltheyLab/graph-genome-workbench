...
def plot_concordances_all(files, outname, sources):
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			plt.scatter(x_values, concordances, label=source, color=colors[i], marker='o', s=concordances)
		#plt.ylim(0,100)
	...
	for i, source in enumerate(sources):
		...
		handles.append(line)
		labels.append(label)
	plt.figlegend(handles, labels, loc='upper right', bbox_to_anchor=(0.8, 1.08), ncols=len(labels))
...
def plot_untyped_all(files, outname, sources):
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			is_first = False
			x_values = [i*5 for i in range(len(samples))]
			plt.scatter(x_values, untyped, label=source, color=colors[i], marker='o', s=50)
		#plt.ylim(0.0, 100.0)
		plt.title(var_to_name[var])
		plt.xticks(x_values, all_samples, rotation='vertical')
...
def plot_fscores_all(files, outname, sources):	
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			is_first = False
			x_values = [i*5 for i in range(len(samples))]
			plt.scatter(x_values, fscores, label=source, color=colors[i], marker='o', s=100)
		#plt.ylim(0,100)
		plt.title(var_to_name[var])
		plt.xticks(x_values, all_samples, rotation='vertical')
...
def plot_concordance_vs_untyped(files, outname, sources):
	...
	colors = ['#377eb8', '#ff7f00', '#4daf4a', '#f781bf', '#a65628', '#984ea3', '#808080', '#f7ce37', '#bc202c', '#251188']
	type_to_file = {}
	n_cols = 4 # one col per variant type
	plt.figure(figsize=(15,3.5))
	for source in sources:
		for f in files:
			if not ('-' + source + '_') in f:
	...
	for var in variants:
		for sample in all_samples:
			plt.subplot(len(all_samples), n_cols, plot_index)
			for i,source in enumerate(sources):
				if (var, source, sample) in data:
					plt.scatter(data[(var, source, sample)][1], data[(var, source, sample)][0],  marker='o', label=source, color=colors[i], s=(data[(var, source, sample)][1] + data[(var, source, sample)][0]))
			#plt.xlim(0,100)
			#plt.ylim(0,100)
