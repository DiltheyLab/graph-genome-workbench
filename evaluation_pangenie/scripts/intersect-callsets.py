...
#    assert all([c.vartype == cluster[0].vartype for c in cluster])
    ...
    for c in callsets:
        if callset_to_variant[c] != []:
            ...
            af = callset_to_variant[c][0].af
            break

    callset_to_id = ['nan'] * len(callsets)
    callset_to_present = ['False'] * len(callsets)
    callset_to_af = ['nan'] * len(callsets)
    for i, c in enumerate(callsets):
        if callset_to_variant[c] != []:
            callset_to_id[i] = ','.join([v.id for v in callset_to_variant[c]])
            callset_to_present[i] = 'True'
            callset_to_af[i] = ','.join([v.af for v in callset_to_variant[c]])
    tab_record = [var_id, chrom, str(start), str(end), str(length), vartype, qual, af] + callset_to_present + callset_to_id + callset_to_af
...
class Variant:
    ...
    def __init__(self, chrom, start, end, varlen, vartype, callset, quality, allele_freq="nan"):
        self.chrom = chrom
        self.start = start
        self.end = end
        self.varlen = varlen
    ...
    def __hash__(self):
        return hash((self.chrom, self.start, self.end, self.varlen, self.vartype, self.callset, self.quality))
...
def parse_vcf(filename, name, variants, vartypes, id_from_vcf=False):
    ...
    for line in open(filename, 'r'):
        ...
        if len(alleles) > 1:
            sys.stderr.write('Mulit-allelic position ' + chrom + ':' + str(start) + ' in ' + filename + '.\n')

        info_field = {i.split('=')[0]:i.split('=')[1] for i in fields[7].split(';') if '=' in i if '=' in i}

        if 'SVTYPE' in info_field:
            vartype = info_field['SVTYPE']
        elif 'ID' in info_field:
            var_id = info_field['ID']
            vartype = var_id.split('-')[2]
            assert vartype in ['INS', 'DEL']
        else:
            sys.stderr.write('No information on variant type for ' + chrom + ':' + str(start) + ' in ' + filename + '.\n')
            sys.exit(1)
        ...
        if not 'SVLEN' in info_field:
            # VCF must be bi-allelic
            assert not ',' in fields[4]
            assert 'ID' in info_field
            varlen = int(info_field['ID'].split('-')[-1])
        else:
        ...
        if 'AF' in info_field:
            af = info_field['AF']
        variant = Variant(chrom, start, end, varlen, vartype, name, quality, af)
        if id_from_vcf:
            variant.id = fields[2]
...
def parse_variant_bed(filename, name, variants, vartypes):
    """
       Reads a BED file encoding SV calls.
    """
    var_counter = 0
    for line in open(filename, 'r'):
        if line.startswith('#'):
        ...
        varlen = int(fields[3])
        vartype = fields[4]
        variant = Variant(chrom, start, end, varlen, vartype, name, '.', "nan")
        variants.append(variant)
        vartypes.add(vartype)
        var_counter += 1
    return var_counter
...
def parse_table(filename, variants, fulltable):
    ...
    for line in open(filename, 'r'):
        ...
        quality = fields[6]
        af = fields[7]
        variant = Variant(chrom, start, end, varlen, vartype, 'table', quality, af)
...
def run_intersect(callsets, names, outtable, outvcf, outpdf):
    ...
    for callset, name in zip(callsets, names):
        ...
        elif callset.endswith('.bed'):
            count = parse_variant_bed(callset, name, variants, vartypes)
        else:
            assert(False)
    ...
    with open(outtable, 'w') as outtable, open(outvcf, 'w') as outvcf:
        tsv_header = [ 'ID', 'chromosome', 'start', 'end', 'length', 'var_type', 'quality', 'allele_freq'] + ['in_' + c for c in names] + ['ID_' + c for c in names] + ['AF_' + c for c in names]
...
if __name__ == '__main__':
    ...
    parser_intersect.add_argument('-c', '--callsets', nargs='+', required=True, default=[], help='Callsets in VCF or BED format. Order corresponds to merging order.')
